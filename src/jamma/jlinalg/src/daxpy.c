 ...
 *
 * Implements:
 *   jlinalg_daxpy_generic — portable scalar with 4x unroll for unit strides
 *   jlinalg_daxpy_avx2    — x86_64 AVX2 with broadcast alpha and FMA unroll
 *
        ...
        double       *y, int incy)
{
    ...
    if (incx != 1 || incy != 1) {
        jlinalg_daxpy_generic(n, alpha, x, incx, y, incy);
        return;
    }
