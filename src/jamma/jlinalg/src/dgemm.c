/**
 * dgemm.c — Goto/BLIS three-level blocking dgemm for jlinalg.
 *
 ...
 *
 * Blocking parameters and workspace buffers are set by jlinalg_dgemm_init(),
 * which is called from jlinalg_init() in platform.c after ISA detection.
 *
 * Thread safety:
 *   - A pthread mutex serialises concurrent callers (jlinalg_packed_B is a single
 *     shared buffer written outside the OpenMP region).
 *   - jlinalg_packed_A is per-thread: each thread computes its own packed_A
 *     offset from omp_get_thread_num().
 *   - OpenMP parallel-for over the IC loop.
 *   - The parallel-for is clamped to jlinalg_n_threads (the thread count at init
 ...
 * threads can enter jlinalg_dgemm_c simultaneously.  Both packed_B (shared across
 ...
 * the shared jlinalg_packed_B workspace). */
pthread_mutex_t jlinalg_dgemm_mutex = PTHREAD_MUTEX_INITIALIZER;
...
/* ---------------------------------------------------------------------------
 * Global dgemm state — set once by jlinalg_dgemm_init(), then read-only
 * ---------------------------------------------------------------------------
 */
int JLINALG_MR = 4;
int JLINALG_NR = 4;
...
double *jlinalg_packed_A = NULL;
double *jlinalg_packed_B = NULL;
int     jlinalg_n_threads = 1;

/* Microkernel function pointer — set to generic here; overwritten by
 * platform.c after ISA detection. */
jlinalg_dgemm_micro_fn jlinalg_dgemm_microkernel = jlinalg_dgemm_micro_generic;

/* ---------------------------------------------------------------------------
 * jlinalg_dgemm_init — Validate blocking invariants and allocate packing workspace.
 *
 * Called from jlinalg_init() in platform.c after ISA detection.
 * ISA-specific blocking params are set in platform.c before this is called,
 * so this function uses the already-set JLINALG_MR/NR/KC/MC/NC values.
 *
...
int jlinalg_dgemm_init(void) {
    ...
    if (JLINALG_MR > MAX_MR || JLINALG_NR > MAX_NR) {
        fprintf(stderr,
            "FATAL: JLINALG_MR=%d > MAX_MR=%d or JLINALG_NR=%d > MAX_NR=%d\n",
            JLINALG_MR, MAX_MR, JLINALG_NR, MAX_NR);
        return -1;
    }
    ...
    if (JLINALG_NC % JLINALG_NR != 0 || JLINALG_MC % JLINALG_MR != 0) {
        fprintf(stderr,
            "FATAL: blocking invariant violated: NC=%d must be divisible by NR=%d, "
            "MC=%d must be divisible by MR=%d\n",
            JLINALG_NC, JLINALG_NR, JLINALG_MC, JLINALG_MR);
        return -1;
    }

#ifdef _OPENMP
    jlinalg_n_threads = omp_get_max_threads();
    if (jlinalg_n_threads < 1) jlinalg_n_threads = 1;
#else
    jlinalg_n_threads = 1;
#endif
    ...
    size_t b_bytes = ALIGN_UP(
        (size_t)JLINALG_KC * (size_t)JLINALG_NC * sizeof(double), 64);
    jlinalg_packed_B = (double *)aligned_alloc(64, b_bytes);
    if (!jlinalg_packed_B) {
        fprintf(stderr, "jlinalg: aligned_alloc failed for packed_B (%zu bytes)\n", b_bytes);
        return -1;
    }

    /* packed_A: n_threads * MC * KC doubles — per-thread slice */
    size_t a_bytes = ALIGN_UP(
        (size_t)jlinalg_n_threads * (size_t)JLINALG_MC * (size_t)JLINALG_KC * sizeof(double), 64);
    jlinalg_packed_A = (double *)aligned_alloc(64, a_bytes);
    if (!jlinalg_packed_A) {
        fprintf(stderr, "jlinalg: aligned_alloc failed for packed_A (%zu bytes)\n", a_bytes);
        free(jlinalg_packed_B);
        jlinalg_packed_B = NULL;
        return -1;
    }
...
/* ---------------------------------------------------------------------------
 * jlinalg_dgemm_cleanup — Free workspace buffers.
 *
 * Safe to call on NULL pointers. Called from module cleanup if needed.
 * ---------------------------------------------------------------------------
 */
void jlinalg_dgemm_cleanup(void) {
    free(jlinalg_packed_A);
    free(jlinalg_packed_B);
    jlinalg_packed_A = NULL;
    jlinalg_packed_B = NULL;
}

/* ---------------------------------------------------------------------------
 * jlinalg_pack_A — Pack a mc x kc submatrix of A into MR-wide column strips.
 *
...
/* ---------------------------------------------------------------------------
 * jlinalg_pack_B — Pack a kc x nr subpanel of B into NR-wide column strips.
 *
 * Layout in packed buffer:
 ...
 *   nr_param — tile width (JLINALG_NR)
 *   trans    — 0=no transpose, 1=transpose
 * ---------------------------------------------------------------------------
 */
    ...
    for (npy_intp jc = 0; jc < N; jc += NC) {
        ...
        for (npy_intp pc = 0; pc < K; pc += KC) {
            ...
            const double *B_panel = transb ? (B + jc * ldb + pc)
                                           : (B + pc * ldb + jc);
            jlinalg_pack_B(B_panel, ldb, kc_actual, nr_actual,
            ...
            for (npy_intp ic = 0; ic < M; ic += MC) {
                ...
                const double *A_panel = transa ? (A + pc * lda + ic)
                                               : (A + ic * lda + pc);
                jlinalg_pack_A(A_panel, lda, mc_actual, kc_actual,
                ...
                for (npy_intp jr_s = 0; jr_s < n_nr_strips; jr_s++) {
                    ...
                    for (npy_intp ir_s = 0; ir_s < n_mr_strips; ir_s++) {
                        ...
                        if (mr_tile == MR && nr_tile == NR) {
                            jlinalg_dgemm_microkernel(kc_actual, pA_strip, pB_strip,
                                                    C_tile, ldc);
                        } else {
...
/* ---------------------------------------------------------------------------
 * jlinalg_dgemm_c — Three-level blocking DGEMM: C = op(A) * op(B).
 *
                   ...
                   int transa, int transb)
{
    if (!jlinalg_packed_A || !jlinalg_packed_B) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_c called but workspace not allocated "
            "(jlinalg_dgemm_init() failed or was never called)\n");
        abort();
    }

    int lock_err = pthread_mutex_lock(&jlinalg_dgemm_mutex);
    if (lock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_c: pthread_mutex_lock failed (errno=%d)\n",
            lock_err);
        abort();
    }
    ...
    _dgemm_core(M, N, K, A, lda, B, ldb, C, ldc,
                ...
                jlinalg_packed_A, jlinalg_packed_B, jlinalg_n_threads);

    int unlock_err = pthread_mutex_unlock(&jlinalg_dgemm_mutex);
    if (unlock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_c: pthread_mutex_unlock failed (errno=%d)\n",
            unlock_err);
        abort();
    }
}
...
/* ---------------------------------------------------------------------------
 * jlinalg_dgemm_accum_c — Accumulate GEMM: C = alpha * op(A) * op(B) + beta * C.
 *
                          ...
                          double alpha, double beta)
{
    if (!jlinalg_packed_A || !jlinalg_packed_B) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_accum_c called but workspace not allocated\n");
        abort();
    }
    ...
    if (lock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_accum_c: pthread_mutex_lock failed (errno=%d)\n",
            lock_err);
        abort();
    }
    ...
    if (unlock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_accum_c: pthread_mutex_unlock failed (errno=%d)\n",
            unlock_err);
        abort();
    }
}
...
/* ---------------------------------------------------------------------------
 * jlinalg_dgemm_ws — Workspace-explicit GEMM (no mutex).
 *
                    ...
                    jlinalg_workspace_t *ws)
{
    if (!ws || !ws->packed_A || !ws->packed_B) {
        fprintf(stderr,
            "FATAL: jlinalg_dgemm_ws called with NULL workspace\n");
        abort();
    }
...
/* ---------------------------------------------------------------------------
 * jlinalg_workspace_alloc — Allocate a caller-managed GEMM workspace.
 *
...
int jlinalg_workspace_alloc(jlinalg_workspace_t *ws, int n_threads) {
    ...
    size_t a_bytes = ALIGN_UP(
        (size_t)n_threads * (size_t)JLINALG_MC * (size_t)JLINALG_KC * sizeof(double), 64);
...
/* ---------------------------------------------------------------------------
 * jlinalg_workspace_free — Free workspace buffers and zero the struct.
 * ---------------------------------------------------------------------------
 */
...
/* ---------------------------------------------------------------------------
 * _dgemm_dispatch — Simplified no-transpose wrapper matching jlinalg_dgemm_fn
 * signature for the dispatch table (jlinalg_dispatch.dgemm).
 *
 * Assumes row-major layout with natural leading dimensions (lda=K, ldb=N,
 * ldc=N).  Always passes transa=transb=0 (no transpose).
 *
 * Note: py_dgemm and dstedc use jlinalg_dgemm_ext() / jlinalg_dgemm_ext_ws()
 * which route to external BLAS when available, falling back to jlinalg_dgemm_c.
 * ---------------------------------------------------------------------------
 */
                            ...
                            const double *A, const double *B, double *C)
{
    ...
    jlinalg_dgemm_c(m, n, k, A, k, B, n, C, n, 0, 0);
}

/* Expose for platform.c to assign to jlinalg_dispatch.dgemm */
jlinalg_dgemm_fn jlinalg_dgemm_dispatch_fn = _dgemm_dispatch;
