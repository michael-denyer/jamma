 ...
 *
 * Implements:
 *   jlinalg_dgemv_generic — row-by-row calls to jlinalg_ddot_generic
 *   jlinalg_dgemv_avx2    — row-by-row calls to jlinalg_ddot_avx2
 *
 ...
 * table) so they are self-contained — no dependency on jlinalg_init().
 */
        ...
        double       *y)
{
    ...
    for (npy_intp i = 0; i < m; i++)
        y[i] = jlinalg_ddot_generic(n, A + i * n, 1, x, 1);
}

/* ---------------------------------------------------------------------------
 * AVX2 implementation (x86_64 only) — row-by-row jlinalg_ddot_avx2
 *
 * Same structure as generic but calls jlinalg_ddot_avx2 directly for SIMD
        ...
        double       *y)
{
    ...
    for (npy_intp i = 0; i < m; i++)
        y[i] = jlinalg_ddot_avx2(n, A + i * n, 1, x, 1);
}
