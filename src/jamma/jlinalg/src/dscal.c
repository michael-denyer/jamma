 ...
 *
 * Implements:
 *   jlinalg_dscal_generic — portable scalar with alpha=0/1 special cases
 *   jlinalg_dscal_avx2    — x86_64 AVX2 with broadcast alpha and vectorised multiply
 *
 * Special cases:
        ...
        double *x, int incx)
{
    ...
    if (incx != 1) {
        jlinalg_dscal_generic(n, alpha, x, incx);
        return;
    }
