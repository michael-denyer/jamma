/**
 * dstedc.c — Divide-and-conquer tridiagonal eigensolver for jlinalg.
 *
 * Implements jlinalg_dstedc_c: computes all eigenvalues and eigenvectors of a
 * real symmetric tridiagonal matrix T given by diagonal d[N] and off-diagonal
 * e[N-1].  Z is caller-allocated N x N (jlinalg_dstedc_c initializes it to
 ...
 *   jlinalg uses row-major layout throughout.  Here Z[N x N] row-major means
 ...
 *        Uses jlinalg_dgemm_c for the N x N matrix multiply.
 *
...
static void sort_eig(double *d, double *Z, npy_intp ldz, npy_intp n)
{
    ...
    if (!idx || !d_tmp || !z_col) {
        ...
        if (n > 10000) {
            fprintf(stderr, "jlinalg dstedc: sort_eig allocation failed for "
                    "n=%ld, too large for O(n^2) insertion sort fallback\n",
                    (long)n);
            return;  /* caller will see unsorted eigenvalues */
        }
        fprintf(stderr, "jlinalg dstedc: sort_eig allocation failed for n=%ld, "
 ...
 * ---------------------------------------------------------------------------
 */
static int dgemm_dispatch(npy_intp M, npy_intp N, npy_intp K,
...
{
    if (ws) {
        jlinalg_dgemm_ext_ws(M, N, K, A, lda, B, ldb, C, ldc,
                           transa, transb, alpha, beta, ws);
        return 0;
    } else {
        /* Global-mutex path: jlinalg_dgemm_ext only supports alpha=1, beta=0.
         * All merge_rank1 calls use those values. */
        if (alpha != 1.0 || beta != 0.0) {
            fprintf(stderr, "ERROR: dgemm_dispatch: non-ws path requires "
                    "alpha=1, beta=0 but got alpha=%.1f, beta=%.1f\n",
                    alpha, beta);
            return JLINALG_EXT_INTERNAL_ERROR;
        }
        jlinalg_dgemm_ext(M, N, K, A, lda, B, ldb, C, ldc, transa, transb);
        return 0;
    }
}
                       ...
                       jlinalg_eigh_status_t *status)
{
    ...
    for (npy_intp i = 0; i < n_nd; i++) {
        ...
        if (info != 0) {
             ...
             * jlinalg_dstedc_c will trigger QR fallback if results are bad. */
        }
    }
    if (n_secular_failures > 0) {
        fprintf(stderr, "jlinalg dstedc: %d/%ld secular equation(s) "
    ...
    for (npy_intp k = 0; k < n_nd; k++) {
        ...
        for (npy_intp j = 0; j < n_nd; j++) {
            ...
            if (fabs(den) < DBL_MIN) {
                ...
                fprintf(stderr, "jlinalg dlaed3: near-zero denominator at k=%ld j=%ld "
        ...
        if (w > 0.0) {
            ...
            if (w > 1e-100) {
                fprintf(stderr, "jlinalg dlaed3: positive weight product w=%.2e "
    ...
    if (n_nd > 0) {
        ...
        int gemm_ret = dgemm_dispatch(n, n_nd, n_nd,
    ...
    int gemm_ret2 = dgemm_dispatch(n, n, n,
...
/* ---------------------------------------------------------------------------
 * jlinalg_dstedc_c — Public D&C tridiagonal eigensolver.
 *
                   ...
                   jlinalg_eigh_status_t *status)
{
    ...
    int need_fallback = 0;
    if (ret != 0) {
        fprintf(stderr, "jlinalg dstedc: D&C returned %d (N=%ld), "
                "attempting QR fallback\n", ret, (long)N);
        need_fallback = 1;
    } else {
        double resid = tridiag_eig_residual(N, d_orig, e_orig, d, Z, ldz);
        if (resid > 1e-8) {
            fprintf(stderr, "jlinalg dstedc: D&C residual %.2e (N=%ld), "
                    "attempting QR fallback\n", resid, (long)N);
            need_fallback = 1;
        } else if (resid > 1e-14) {
            fprintf(stderr, "jlinalg dstedc: D&C residual %.2e (N=%ld) -- "
    ...
    if (need_fallback) {
        ...
        if (ret == -1) {
            fprintf(stderr, "jlinalg dstedc: D&C allocation failure (N=%ld), "
                    "skipping QR fallback (same allocation will fail)\n",
                    (long)N);
        } else if (N > 10000) {
            fprintf(stderr, "jlinalg dstedc: D&C residual too high but N=%ld "
                    "is too large for O(N^3) QR fallback — returning error\n",
                    (long)N);
            ret = -4;  /* JLINALG_EXT_INTERNAL_ERROR */
        } else {
            if (status) status->qr_fallback = 1;
            if (N > 2000) {
                fprintf(stderr, "jlinalg dstedc: QR fallback on N=%ld — "
