/**
 * dsyrk.c — Symmetric rank-k update for jlinalg.
 *
 ...
 *       Pack B panel from X.T for this (PC, JC) pair (shared jlinalg_packed_B).
 ...
 *
 * Workspace:
 *   Uses jlinalg_packed_B (KC * NC doubles) — never overflows because JC loop
 *   partitions N into NC-wide panels (Pitfall 1 prevention).
 *   Uses jlinalg_packed_A (per-thread MC * KC doubles).
 *
 * Thread safety:
 *   Acquires jlinalg_dgemm_mutex (shared with dgemm.c and dsyr2k.c) to
 *   serialise access to jlinalg_packed_B across concurrent callers.
 *
 * Mirror step:
                         ...
                         int n_threads)
{
    ...
    for (npy_intp pc = 0; pc < K; pc += KC) {
        ...
        for (npy_intp jc = 0; jc < N; jc += NC) {
            ...
            jlinalg_pack_B(X + jc * ldx + pc, ldx,
            ...
            for (npy_intp ic = 0; ic < N; ic += MC) {
                ...
                jlinalg_pack_A(X + ic * ldx + pc, ldx,
...
/* ---------------------------------------------------------------------------
 * jlinalg_dsyrk_c — Symmetric rank-k update: C = X @ X.T (lower then mirror)
 *
                   ...
                   double *C, npy_intp ldc)
{
    ...
    if (N < 0 || K < 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_c: negative dimension N=%ld K=%ld\n",
            (long)N, (long)K);
        abort();
    }
    ...
    if (!jlinalg_packed_A || !jlinalg_packed_B) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_c called but workspace not allocated "
    ...
    if (lock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_c: pthread_mutex_lock failed (errno=%d)\n",
            lock_err);
        abort();
    }
    ...
    if (unlock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_c: pthread_mutex_unlock failed (errno=%d)\n",
            unlock_err);
        abort();
    }
...
/* ---------------------------------------------------------------------------
 * jlinalg_dsyrk_ws — Workspace-explicit symmetric rank-k update (no mutex).
 *
 * Same computation as jlinalg_dsyrk_c but uses caller-owned workspace.
                     ...
                     jlinalg_workspace_t *ws)
{
    if (N < 0 || K < 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_ws: negative dimension N=%ld K=%ld\n",
            (long)N, (long)K);
        abort();
    }
    ...
    if (!ws || !ws->packed_A || !ws->packed_B || ws->n_threads < 1) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_ws called with invalid workspace "
            "(n_threads=%d)\n", ws ? ws->n_threads : -1);
        abort();
    }
                          ...
                          double *C, npy_intp ldc)
{
    ...
    if (N < 0 || K < 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_c: negative dimension N=%ld K=%ld\n",
            (long)N, (long)K);
        abort();
    }
    ...
    if (!jlinalg_packed_A || !jlinalg_packed_B) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_c called but workspace not allocated "
            "(jlinalg_dgemm_init() failed or was never called)\n");
        abort();
    }
    ...
    if (lock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_c: pthread_mutex_lock failed (errno=%d)\n",
            lock_err);
        abort();
    }
    ...
    if (unlock_err != 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_c: pthread_mutex_unlock failed (errno=%d)\n",
            unlock_err);
        abort();
    }
...
/* ---------------------------------------------------------------------------
 * jlinalg_dsyrk_lower_ws — Workspace-explicit lower-only rank-k update (no mutex).
 *
 * Same as jlinalg_dsyrk_lower_c but uses caller-owned workspace.
 * ---------------------------------------------------------------------------
 */
                            ...
                            jlinalg_workspace_t *ws)
{
    if (N < 0 || K < 0) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_ws: negative dimension N=%ld K=%ld\n",
            (long)N, (long)K);
        abort();
    }
    ...
    if (!ws || !ws->packed_A || !ws->packed_B || ws->n_threads < 1) {
        fprintf(stderr,
            "FATAL: jlinalg_dsyrk_lower_ws called with invalid workspace "
            "(n_threads=%d)\n", ws ? ws->n_threads : -1);
        abort();
    }
