                         ...
                         double *mirror_buf)
{
    ...
    jlinalg_dispatch.dgemv(n, n, mirror_buf, x_buf, y);
...
/* jlinalg_dsytrd_c — Blocked Householder tridiagonalization (public API).
 *
 ...
 *     If non-NULL, uses jlinalg_dsyr2k_ws (no mutex contention).
 *     If NULL, falls back to jlinalg_dsyr2k_c (global mutex path).
 *
                   ...
                   jlinalg_eigh_status_t *status)
{
    ...
    if (!mirror_buf) {
        fprintf(stderr, "jlinalg dsytrd: mirror buffer allocation failed "
