...
def run_lmm_association_numpy(
    ...
    # These are standalone fused paths (not workspace-based like Wald fused).
    use_fused_score = (
    ...
    # Extract w for fused paths that need it (Wald fused extracts inside
    # workspace creation; Score/LRT fused are stateless so extract here).
    w = (
        UtW[:, 0].copy()
        ...
        if use_pipeline:
            ...
            for chunk_start in chunk_iterator:
                ...
                if use_fused:
                    ...
                    if use_fused_general:
                        # Mode-4 fused general restricted at dispatch
                        if lmm_mode != 1:
                            raise ValueError(
                                "Mode-4 fused general disabled (NaN lambda_mle bug)"
                            )
