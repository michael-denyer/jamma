"""Scalar Cephes betainc and chi2_sf, the oracles for jamma.lmm.special's batch forms.

Production calls only ``betainc_batch`` and ``chi2_sf_batch``. ``betainc`` is
the one-value port of codeplea/incbeta (zlib license) and shares the batch
form's continued fraction constants; ``chi2_sf`` is the erfc identity for one
degree of freedom.
"""
...
from jamma.lmm.special import _CF_MAX_ITER, _CF_STOP, _CF_TINY
...
def betainc(a: float, b: float, z: float, complement_z: float | None = None) -> float:
    ...
    Returns:
        I_z(a, b) in [0, 1], or NaN when the continued fraction does not
        converge, matching betainc_batch.
