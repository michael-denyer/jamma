...
def _silent_report() -> BuildReport:
    return BuildReport(detail=lambda _m: None, warn=lambda _m: None)
...
def test_cpp_does_not_probe_c_or_openmp(monkeypatch) -> None:
    ...
    toolchain = detect_toolchain(_silent_report(), language="c++", uses_openmp=False)
...
def test_cpp_flags_preserve_special_values_and_c_policy() -> None:
    ...
    cpp = resolve_flags(MATRIX_TEXT_SPEC, dev_mode=False, system="Linux", env=env)
    assert cpp.base_extra == ("-Ofast",)
    flags = resolve_cflags_for(cpp, [], lapack=False, cxx=True)
    assert "-std=c++17" in flags
    assert "-std=c11" not in flags
    assert flags.index("-fno-fast-math") > flags.index("-Ofast")
    c = resolve_flags(LMM_ACCEL_SPEC, dev_mode=False, system="Linux", env=env)
    assert c.base_extra == ("-march=x86-64-v3",)
