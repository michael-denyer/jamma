...
``resolve_flags`` for ``LMM_ACCEL_SPEC``.

When ``JAMMA_SENTINEL_UB`` is truthy, ``resolve_flags`` appends
...
``resolve_flags`` is a pure function of the spec and the environment, so
these tests pass ``env`` as a plain dict and assert on the resolved flags. No
...
from jamma._build_support.build_models import LMM_ACCEL_SPEC, resolve_flags
...
def _resolve(env: dict[str, str]) -> tuple[str, ...]:
    return resolve_flags(
        LMM_ACCEL_SPEC, dev_mode=True, system="Linux", env=env
    ).base_extra
