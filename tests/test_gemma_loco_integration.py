...
# ---------------------------------------------------------------------------
# SC-05: Secular update GEMMA parity
# ---------------------------------------------------------------------------


@pytest.mark.tier1
class TestSecularUpdateGemmaParity:
    """Validate secular update path against GEMMA LMM reference.

    Runs run_lmm_loco with use_secular_update=True and verifies that
    results match the GEMMA reference fixtures within validated tolerances.
    Also cross-checks secular vs standard JAMMA results to confirm the
    rotated-basis update is numerically equivalent.
    """

    @pytest.fixture(scope="class")
    def gemma_loco_assoc(self) -> dict[str, pd.DataFrame]:
        """Load GEMMA reference fixtures (shared with TestGemmaLocoValidation)."""
        return {
            chrom: pd.read_csv(path, sep="\t")
            for chrom, path in GEMMA_CHR_ASSOC.items()
        }
    ...
    @pytest.fixture(scope="class")
    def secular_loco_results(self) -> dict[str, pd.DataFrame]:
        """Run JAMMA LOCO with use_secular_update=True, group by chromosome."""
        phenotypes = load_phenotypes_from_fam(PLINK_PREFIX.with_suffix(".fam"))
        loco = run_lmm_loco(
            bed_path=PLINK_PREFIX,
            phenotypes=phenotypes,
    ...
    @pytest.fixture(scope="class")
    def standard_loco_results(self) -> dict[str, pd.DataFrame]:
        """Run JAMMA LOCO with standard path for cross-comparison."""
        phenotypes = load_phenotypes_from_fam(PLINK_PREFIX.with_suffix(".fam"))
        loco = run_lmm_loco(
            bed_path=PLINK_PREFIX,
            phenotypes=phenotypes,
    ...
    def test_secular_all_chromosomes_present(
        self, secular_loco_results: dict[str, pd.DataFrame]
    ):
        """Secular update returns results for all 3 chromosomes."""
        assert set(secular_loco_results.keys()) == {"1", "2", "3"}, (
            f"Expected chromosomes {{1, 2, 3}}, got {set(secular_loco_results.keys())}"
        )

    def test_secular_snp_count_per_chromosome(
        self,
        gemma_loco_assoc: dict[str, pd.DataFrame],
        secular_loco_results: dict[str, pd.DataFrame],
    ):
        """Secular update tests the same number of SNPs as GEMMA per chromosome."""
        for chrom in _CHROMOSOMES:
            n_gemma = len(gemma_loco_assoc[chrom])
            n_secular = len(secular_loco_results[chrom])
            assert n_secular == n_gemma, (
                f"chr{chrom}: secular={n_secular} SNPs, GEMMA={n_gemma} SNPs"
            )

    def test_secular_beta_matches_gemma(
        self,
        gemma_loco_assoc: dict[str, pd.DataFrame],
        secular_loco_results: dict[str, pd.DataFrame],
    ):
        """Secular update beta matches GEMMA within 1e-5 absolute tolerance."""
        for chrom in _CHROMOSOMES:
            merged = pd.merge(
                gemma_loco_assoc[chrom],
                secular_loco_results[chrom],
                on="rs",
                suffixes=("_gemma", "_secular"),
                how="inner",
            )
            max_diff = np.max(np.abs(merged["beta_gemma"] - merged["beta_secular"]))
            assert max_diff < 1e-5, (
                f"chr{chrom}: secular beta max abs diff {max_diff:.2e} >= 1e-5"
            )

    def test_secular_pwald_matches_gemma(
        self,
        gemma_loco_assoc: dict[str, pd.DataFrame],
        secular_loco_results: dict[str, pd.DataFrame],
    ):
        """Secular update p_wald matches GEMMA within 1e-5 absolute tolerance."""
        for chrom in _CHROMOSOMES:
            merged = pd.merge(
                gemma_loco_assoc[chrom],
                secular_loco_results[chrom],
            ...
            max_diff = np.max(np.abs(merged["p_wald_gemma"] - merged["p_wald_secular"]))
            assert max_diff < 1e-5, (
                f"chr{chrom}: secular p_wald max abs diff {max_diff:.2e} >= 1e-5"
            )

    def test_secular_matches_standard_jamma(
        self,
        secular_loco_results: dict[str, pd.DataFrame],
        standard_loco_results: dict[str, pd.DataFrame],
    ):
        """Secular update matches standard JAMMA path within validated tolerances.

        The rotated-basis update should produce numerically equivalent
        association results to direct K_loco eigendecomposition.
        """
        for chrom in _CHROMOSOMES:
            merged = pd.merge(
                standard_loco_results[chrom],
                secular_loco_results[chrom],
                on="rs",
                suffixes=("_std", "_secular"),
                how="inner",
            )
            assert len(merged) == len(standard_loco_results[chrom]), (
                f"chr{chrom}: merge dropped rows — "
                f"standard has {len(standard_loco_results[chrom])} SNPs "
                f"but merge produced {len(merged)}"
            )

            beta_diff = np.max(np.abs(merged["beta_std"] - merged["beta_secular"]))
            pwald_diff = np.max(np.abs(merged["p_wald_std"] - merged["p_wald_secular"]))
            assert beta_diff < 1e-5, (
                f"chr{chrom}: secular vs standard beta diff {beta_diff:.2e} >= 1e-5"
            )
            assert pwald_diff < 1e-5, (
                f"chr{chrom}: secular vs standard p_wald diff {pwald_diff:.2e} >= 1e-5"
            )


# ---------------------------------------------------------------------------
# SC-06: Secular update timing diagnostic
# ---------------------------------------------------------------------------


@pytest.mark.benchmark
@pytest.mark.tier1
def test_secular_update_timing() -> None:
    """Log timing comparison: secular update vs standard path on gemma_loco fixture.

    This is an informational diagnostic — no assertion on timing.
    At n=100 samples the speedup may be negligible; the test documents
    the relative overhead.

    Run with: uv run pytest tests/test_gemma_loco_integration.py -v -n0 -m benchmark
    """
    import time
    ...
    run_lmm_loco(**common_kwargs, use_secular_update=False)
    ...
    t0 = time.perf_counter()
    run_lmm_loco(**common_kwargs, use_secular_update=True)
    t_secular = time.perf_counter() - t0

    ratio = t_standard / t_secular if t_secular > 0 else float("inf")
    print(
        f"\nSecular update: {t_secular:.3f}s, "
        f"Standard: {t_standard:.3f}s, "
