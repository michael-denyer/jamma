...
class TestHardwareContext:
    ...
    def test_hardware_context_keys(self, get_hardware_context):
    ...
    def test_hardware_context_types(self, get_hardware_context):
        """All values are JSON-serializable types."""
        ctx = get_hardware_context()
        # Should not raise
        json.dumps(ctx)

    def test_hardware_context_cpu_model_nonempty(self, get_hardware_context):
        """CPU model is a non-empty string."""
        ctx = get_hardware_context()
        assert isinstance(ctx["cpu_model"], str)
        assert len(ctx["cpu_model"]) > 0

    def test_hardware_context_blas_backend(self, get_hardware_context):
        """BLAS backend is a non-empty string."""
        ctx = get_hardware_context()
        assert isinstance(ctx["blas_backend"], str)
        assert len(ctx["blas_backend"]) > 0

    def test_hardware_context_positive_counts(self, get_hardware_context):
