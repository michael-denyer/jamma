"""Tests for jblas dgemm (matrix-matrix product).
...
Run with -n0 to avoid parallel interference with threading tests:
    uv run pytest tests/test_jblas_dgemm.py -x -n0 -v
"""
...
class TestDgemmInit:
    ...
    def test_import_succeeds(self) -> None:
        """Import jamma.jblas.dgemm without error."""
        from jamma.jblas import dgemm as _dgemm  # noqa: F401
...
def test_dgemm_throughput() -> None:
    """1000x1000 dgemm: jblas must match at least 80% of np.matmul throughput.

    Prints GFLOPS for both implementations.  This test is skipped when the
    C extension is not compiled.

    The 0.8x assertion is only enforced on AVX2 hardware where jblas uses its
    ...
    for _ in range(n_iters):
        dgemm(A, B)
    t_jblas = (time.perf_counter() - t0) / n_iters
    ...
    print(f"\njblas dgemm: {gflops_jblas:.1f} GFLOPS ({t_jblas * 1000:.1f} ms)")
    print(f"np.matmul:   {gflops_numpy:.1f} GFLOPS ({t_numpy * 1000:.1f} ms)")
    print(f"Ratio:       {gflops_jblas / gflops_numpy:.3f} (ISA: {_isa})")
    ...
    if _isa == "AVX2":
        assert ratio >= 0.8, (
            f"jblas dgemm is less than 80% of np.matmul throughput on AVX2: "
