...
class TestDsyr2kTileCount:
    ...
    def test_tile_count_equals_2x_naive(self) -> None:
        ...
        """
        from jamma.jlinalg import (  # type: ignore[import]
            JLINALG_KC,
            JLINALG_MC,
            JLINALG_MR,
            JLINALG_NC,
        ...
        N, K = 200, 100
        MR, NR = JLINALG_MR, JLINALG_NR
        KC, MC = JLINALG_KC, JLINALG_MC
        self._NC = JLINALG_NC
        ...
        assert total_tiles == 2 * naive_tiles, (
            ...
            f"MR={MR}, NR={NR}, KC={KC}, MC={MC}, NC={JLINALG_NC}"
        )
...
class TestWorkspaceExplicitParity:
    ...
    @staticmethod
    def _load_lib():
        """Load the jlinalg shared library via ctypes."""
        import ctypes
        from pathlib import Path

        so_dir = Path(__file__).resolve().parent.parent / "src" / "jamma" / "jlinalg"
        so_files = list(so_dir.glob("_jlinalg*.so")) + list(
            so_dir.glob("_jlinalg*.pyd")
        )
        assert so_files, f"No _jlinalg shared library found in {so_dir}"
        return ctypes.CDLL(str(so_files[0]))

    @staticmethod
    def _alloc_workspace(lib):
        """Allocate a jlinalg_workspace_t via the C API.
        ...
        # jlinalg_workspace_t: { double *packed_B, double *packed_A, int n_threads }
    ...
    def test_dsyrk_ws_parity(self) -> None:
        """jlinalg_dsyrk_ws produces bitwise-identical output to jlinalg_dsyrk_c."""
        import ctypes
    ...
    def test_dsyr2k_ws_parity(self) -> None:
        """jlinalg_dsyr2k_ws produces bitwise-identical output to jlinalg_dsyr2k_c."""
        import ctypes
        ...
        C_ws = C_init.copy()
        ws_bytes, ws_ptr = self._alloc_workspace(lib)
        dsyr2k_ws = lib.jlinalg_dsyr2k_ws
    ...
    def test_dsyrk_lower_ws_parity(self) -> None:
        """dsyrk_lower_ws lower triangle matches dsyrk_ws lower triangle."""
        import ctypes
        ...
        dsyrk_lower_ws = lib.jlinalg_dsyrk_lower_ws
...
class TestDsyrkVendorDispatch:
    ...
    def test_dsyrk_vendor_parity_boundary_sizes(self):
        ...
        from jamma.jlinalg import JLINALG_MR

        rng = np.random.default_rng(789)
        for n in [JLINALG_MR - 1, JLINALG_MR, JLINALG_MR + 1, 1, 2]:
            if n < 1:
                continue
