...
@pytest.mark.tier0
class TestLazyEigenMemoryEstimation:
    """Tests for lazy eigendecomp memory estimation functions."""

    def test_lazy_eigen_post_peak_gb(self):
        """Verify formula: (2*N^2 + 2*N) * 8 / 1e9."""
        from jamma.core.memory import _lazy_eigen_post_peak_gb

        n = 100_000
        expected = (2 * n * n + 2 * n) * 8 / 1e9
        result = _lazy_eigen_post_peak_gb(n)
        assert result == pytest.approx(expected), (
            f"_lazy_eigen_post_peak_gb({n}) = {result:.6f}GB, expected {expected:.6f}GB"
        )

    def test_lazy_eigen_post_peak_small(self):
        """Verify formula at small N=1000."""
        from jamma.core.memory import _lazy_eigen_post_peak_gb
    ...
    def test_lazy_vs_standard_post_rotation_memory(self):
        """Lazy post-rotation memory < standard path (no persistent U).

        Standard path: U (N^2) persists through entire LMM phase.
        Lazy path after freeing K_householder + V: only eigenvalues (N).
        """
        from jamma.core.memory import _lazy_eigen_post_peak_gb, _square_matrix_gb

        n = 100_000
        # Standard path keeps U (N^2) throughout
        standard_eigvec_gb = _square_matrix_gb(n)
        # Lazy post-peak keeps K_householder + V + tau + eigenvalues
        lazy_post_peak = _lazy_eigen_post_peak_gb(n)
        # During rotation, lazy holds 2N^2 (more than standard N^2)
        assert lazy_post_peak > standard_eigvec_gb, (
            "During rotation phase, lazy holds 2N^2 > standard N^2"
        )
        # But after freeing everything, only eigenvalues remain (N * 8 bytes)
        lazy_final = n * 8 / 1e9
        assert lazy_final < standard_eigvec_gb, (
            "After freeing all lazy state, only eigenvalues remain (<<< U)"
        )

    def test_lazy_eigen_peak_same_as_standard(self):
        """Eigendecomp peak is the same for lazy and standard (D&C dominates)."""
        from jamma.core.memory import (
            _dsyevd_peak_gb,
            estimate_lmm_lazy_memory,
        )

        n = 10_000
        eigen_peak = _dsyevd_peak_gb(n)
        lazy_peak = estimate_lmm_lazy_memory(n, chunk_size=1000)
        # At N=10000, eigendecomp peak (K + U + workspace ~3N^2)
        # dominates rotation phase (2N^2 + N*chunk), so peaks should be equal
        assert lazy_peak == pytest.approx(eigen_peak), (
            f"Lazy peak ({lazy_peak:.4f}GB) should equal eigendecomp peak "
            f"({eigen_peak:.4f}GB) since D&C workspace dominates"
        )

    def test_lazy_lmm_memory_returns_float(self):
        """estimate_lmm_lazy_memory returns a positive float."""
        from jamma.core.memory import estimate_lmm_lazy_memory

        result = estimate_lmm_lazy_memory(1000, chunk_size=500)
        assert isinstance(result, float)
        assert result > 0

    def test_lazy_lmm_memory_includes_lmm_working_set(self):
        """Lazy estimate includes Uab/Iab batches and rotated vectors."""
        from jamma.core.memory import estimate_lmm_lazy_memory

        n = 1000
        chunk = 500
        # With more covariates, Uab/Iab grow (n_index increases)
        est_cvt1 = estimate_lmm_lazy_memory(n, chunk_size=chunk, n_cvt=1)
        est_cvt3 = estimate_lmm_lazy_memory(n, chunk_size=chunk, n_cvt=3)
        assert est_cvt3 > est_cvt1, (
            "More covariates should increase memory estimate "
            f"(n_cvt=1: {est_cvt1:.6f}GB, n_cvt=3: {est_cvt3:.6f}GB)"
        )
